...
namespace XMLReader {
    ...
    void XmlReader::read(std::shared_ptr<Simulation> &sim, std::shared_ptr<LinkedCellStrategy> &lc) {
        ...
        sim_p.parsers(double_p, double_p, double_p, double_p, double_p, double_p, string_p, int_p, double_p, int_p);
        ...
        fr_check.init(lc);
        memb_p.init(lc, sim);
        parser.parsers(cub_p, sim_p, cub_in_p, sph_p, sph_in_p, bou_in_p, temp_p, fr_check, memb_p);
        parser.pre();
        try {
