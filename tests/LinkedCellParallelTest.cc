...
protected:
    std::shared_ptr<LinkedCellParallel> lc;

    void SetUp() override {
        ParticleGenerator<LinkedCellParallel> gen{};
...
TEST_F(LinkedCellParallelTest, AddTest) {
    EXPECT_EQ(lc->size(), 27);
    EXPECT_EQ((*lc)[2][6].size(), 1);
    EXPECT_EQ((*lc)[2][12].size(), 1);
}

TEST_F(LinkedCellParallelTest, BoundaryTest) {
...
TEST_F(LinkedCellParallelTest, BoundaryTest1) {
    LinkedCell3D::addPeriodic(Boundary::BACK);
    LinkedCell3D::addPeriodic(Boundary::FRONT);
...
TEST_F(LinkedCellParallelTest, MoveTest) {
    LinkedCell3D::addPeriodic(Boundary::BACK);
    LinkedCell3D::addPeriodic(Boundary::FRONT);
    Simulation sim(1, 1);
...
TEST_F(LinkedCellParallelTest, APPTest) {
    int i = 0;
...
TEST_F(LinkedCellParallelTest, MoveTest1) {
    LinkedCell3D::addPeriodic(Boundary::BACK);
    LinkedCell3D::addPeriodic(Boundary::FRONT);
    Simulation sim(1, 1);
    std::shared_ptr<LinkedCellDataStructure> test = lc;
